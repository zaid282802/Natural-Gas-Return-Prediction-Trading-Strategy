...
\begin{itemize}[leftmargin=*]
    \item \textbf{Model Performance}: OLS regression with fundamental factors achieves 54\% improvement over ARMA(2,1) baseline (SSR: 2.57 vs 5.68)
...
\begin{enumerate}
    ...
    \item \textbf{Parsimony Trade-off}: The 6-variable model retains 63\% of the full model's explanatory power ($R^2$ 0.356 vs 0.565) while using only 26\% of parameters
...
\textbf{Residual (Unexplained)} & & & \textbf{64.9\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Figure \ref{fig:correlation} presents the correlation matrix for the six factors, revealing moderate to high pairwise correlations among several variables. As discussed in Section 4.4, multicollinearity does not invalidate the model for prediction purposes.
